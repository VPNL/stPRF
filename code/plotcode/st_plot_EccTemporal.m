function data = st_plot_EccTemporal(df,binsize)
...
getStuff = {'ecc','window','cv_varexp','sigma'};
% getStuff = {'ecc','temporal','cv_varexp'};
...
if ~isfield(roi,'temporal') 
    roi.temporal = roi.window;
end
roi.temporal = roi.temporal(:,1);
...
% thresh.ecc = [0 eccMax] +[0.1 0]; % * binsize/2; 
...
thresh.sig = [0.1 12]; 
% thresh.sig = [0 temporalMax]; 

%--- plotting parameters
xaxislim = [0 eccMax];

% find useful data given thresholds
% roi.cv_varexp = ones(size(roi.cv_varexp ));
ii = roi.cv_varexp > thresh.varexp & ...
     roi.ecc > thresh.ecc(1) & roi.ecc < thresh.ecc(2);

% weighted linear regression:
roi.p = linreg(roi.ecc(ii),roi.temporal(ii),roi.cv_varexp(ii));
...
if exist('bootstrp','file') 
    B = bootstrp(1000,@(x) localfit(x,roi.ecc(ii),roi.temporal(ii),roi.cv_varexp(ii)),(1:numel(roi.ecc(ii))));
    B = B';
    pct1 = 100*0.05/2;
    pct2 = 100-pct1;
...
for b=thresh.ecc(1):binsize:thresh.ecc(2),
    ...
    if any(bii),
        ...
        s = wstat(roi.temporal(bii),roi.cv_varexp(bii));
        % store
        ii2 = find(data.x==b);
        data.y(ii2) = s.mean;
