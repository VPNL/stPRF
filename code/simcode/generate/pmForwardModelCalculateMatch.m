function DTcalc = pmForwardModelCalculateMatch(DTDT,varargin)
...
for nn=1:nchcks
    ...
    for ii=1:DTheight{nn}
        ...
        pm.compute;
        %          plot(pm.BOLDnoise); hold on
        % high SNR:  6  +-(.01)
        % mid  SNR: -3  +-(.01)
        % low  SNR: -6  +-(.01)
        if contains(outputdir,'noise0')
            matchSNR = 0;
        else
            matchSNR = 1;
        end
        if matchSNR
            if contains(outputdir,'noise1')
                matchRange = [1 1.1];   % effective SNR 1.5
            elseif contains(outputdir,'noise2')
                matchRange = [-0.3 -0.2]; % effective SNR 0
%                 matchRange = [-0.4 -0.3]; % effective SNR 0
            elseif contains(outputdir,'noise3')
                matchRange = [-2 -1.9]; % effective SNR -1.5
            end
            pm.Noise.compute;
            pm.compute;
            pm.SNR; counter =1;
            while ~(matchRange(1) < pm.SNR && pm.SNR < matchRange(2))
                if (pm.SNR < matchRange(2))      % 0.0160
                    %  random numbers in the interval (a,b)
                    %  formula r = a + (b-a).*rand(N,1).
                     pm.Noise.values = pm.Noise.values*(0.995 + (0.999-0.995)*rand(1));
                else
                     pm.Noise.values = pm.Noise.values*(1.005 + (1.001-1.005)*rand(1));
                end
                pm.compute;
                pm.SNR;
                counter = counter+1;
